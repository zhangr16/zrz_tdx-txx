<template>
  <div class="actionsList" v-loading="isLoading">
    <!-- 查看商家定制评价 -->
    <evaluation
      ref="evaluation"
      v-if="showEvaluation"
    ...
    <el-form :inline="true" size="mini">
      ...
      <!-- <el-form-item label="任务类型">
        ...
        >
          ...
          <el-option label="熊抢购任务" value="2"></el-option>
        </el-select>
      </el-form-item> -->
      <el-form-item>
    ...
    <el-tabs @tab-click="handleTabClick">
      ...
      <el-tab-pane>
        <span slot="label">已领取({{ status_counts[1] || 0 }})</span>
      </el-tab-pane>
      <el-tab-pane>
        <span slot="label">已提交({{ status_counts[2] || 0 }})</span>
      </el-tab-pane>
      <el-tab-pane>
        <span slot="label">待审核({{ status_counts[3] || 0 }})</span>
      </el-tab-pane>
      <el-tab-pane>
        <span slot="label">已完成({{ status_counts[4] || 0 }})</span>
      </el-tab-pane>
      <el-tab-pane>
        <span slot="label">售后({{ status_counts[5] || 0 }})</span>
      </el-tab-pane>
    </el-tabs>
    <!-- 表格内容 -->
        ...
        <div class="table_main">
          ...
          <div style="min-width:200px">
            ...
            <main class="gift_info">
              <div v-if="entity.order_type != 3" class="gift_info_left">
                <img :src="entity.sipping_url" alt />
              </div>
              <div v-if="entity.order_type != 3" class="liner"></div>
              <div class="gift_info_right" :class="{'_center': entity.order_type == 3}">
                <ul>
                  <li v-if="entity.order_type == 3">优惠价: ¥{{ entity.current_price }}</li>
                  <li>原价: ¥{{ entity.price }}</li>
                  <li v-if="entity.order_type != 3">应返: ¥{{ entity.price }}</li>
                  <li v-else>应返: ¥{{ entity.price - entity.current_price + (entity.integral/100) }}</li>

                  <li
                    v-if="entity.order_type == 3 && entity.integral != 0"
                  >积分: {{ -1*entity.integral }}</li>
                  <li
                    v-else-if="entity.order_type != 3 && entity.get_integral != 0"
                  >可获积分: {{entity.get_integral}}</li>
                </ul>
              </div>
            </main>
          </div>
          ...
          <div style="min-width:80px">
            <header>所属分类</header>
            <main class="relate_num">
              <span>{{entity.module_type == 'free' ? '限量免单' : '熊抢购'}}</span>
            </main>
          </div>
          <div style="min-width:80px">
            <header>任务状态</header>
            <main class="task_status">{{ status_arr[entity.status - 1] }}</main>
          </div>
          <div style="flex:1">
            <header>操作</header>
            <main class="operates">
...
<script>
import { order_list, order_num } from "@/api/user/orderList";
...
export default {
  ...
  methods: {
    async getData() {
      ...
      let res2 = await order_num(this.formData);
      this.status_counts = res2.data;
