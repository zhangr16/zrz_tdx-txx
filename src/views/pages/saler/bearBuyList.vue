<template>
  <div class="bearBuyList" v-loading="isloading">
    <!-- 完成情况弹窗 -->
    ...
    <ul v-if="tableData.length > 0">
      <li class="_li" v-for="(item, x) in tableData" :key="x">
        <div class="table_title">
          ...
          >商家用户名：{{name.avatar}}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;提交日期：{{item.create_time}}</span>
          ...
          <div style="width:180px">
            ...
            <main class="task_cost">
              <ul>
                <li>
                  总金额
                  <i>¥{{item.guarantee_fee + item.service_fee}}</i>
                </li>
                <li>
                  担保金额
  ...
  methods: {
    async getListData(flag) {
      ...
      if (flag) {
        ...
        downloadElement.download = "熊抢购活动列表.xls";
    ...
    rehandleTask(item, bol) {
      if (item) {
        if (bol) {
          this.$router.push("/saler/bearBuyApply?pa_id=" + item.pa_id + "&e=1");
        } else {
          this.$router.push("/saler/bearBuyApply?pa_id=" + item.pa_id);
        }
      }
    },
